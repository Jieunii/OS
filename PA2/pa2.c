...
pick_next:
    if(!list_empty(&readyqueue)){
        int min_lifespan=list_first_entry(&readyqueue, struct process, list)->lifespan;
        list_for_each_entry_safe(p, tmp, &readyqueue, list){
            if(min_lifespan > p->lifespan){
                min_lifespan = p->lifespan;
                list_move(&p->list, &readyqueue);
            }
        }
        next = list_first_entry(&readyqueue, struct process, list);
        list_del_init(&next->list);
...
static struct process *srtf_schedule(void)
{
	...
	if (current->age < current->lifespan) {
        list_for_each_entry_safe(p, tmp, &readyqueue, list){
            if((current->lifespan)-(current->age)>(p->lifespan)-(p->age)){
...
pick_next:
	if (!list_empty(&readyqueue)) {
		int max_prio=list_first_entry(&readyqueue, struct process, list)->prio;
        list_for_each_entry_safe(p, tmp, &readyqueue, list){
            if(max_prio < p->prio){
                max_prio = p->prio;
                list_move(&p->list, &readyqueue);
...
void pip_release(int resource_id)
{
	...
	if (!list_empty(&r->waitqueue)) {
		int maxPrio = list_first_entry(&r->waitqueue, struct process, list)->prio;
		list_for_each_entry_safe(p, tmp, &r->waitqueue, list){
			if(maxPrio < p->prio){
				maxPrio=p->prio;
				list_move(&p->list, &r->waitqueue);
			}
		}
		waiter = list_first_entry(&r->waitqueue, struct process, list);
		assert(waiter->status == PROCESS_WAIT);
